  ...
  return (
    <div className=" min-h-screen bg-[url('../../../Img/abstract.jpg')] bg-cover bg-center">
      ...
      <div className="mt-10">
        ...
        {error ? (
          <p className="text-center mt-5 text-xl text-red-500">
            {" "}
            Error: {error}{" "}
          </p>
        ) : null}
        <ul style={{ listStyleType: "none", padding: 0 }}>
          {data && data.products && data.products.length
            ? data.products.map((item) => (
                <li
                  key={item.id}
                  className="text-white text-center mt-5 text-md"
                >
                  {item.title}
                </li>
              ))
        ...
        <div ref={bottomRef} className="text-center mt-10 mb-10">
          ...
          <h3>
            {" "}
            This is the bottom of page. In order to reach to top click the
            button above.
          </h3>
        </div>
      </div>
